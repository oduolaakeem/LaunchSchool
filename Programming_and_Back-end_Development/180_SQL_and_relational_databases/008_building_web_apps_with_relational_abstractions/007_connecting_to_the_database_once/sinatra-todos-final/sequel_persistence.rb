require "sequel"

DB = Sequel.connect("postgres://ubuntu:ubuntu@localhost/todos")

class SequelPersistence
  def initialize(logger)
    DB.logger = logger
  end
  
  def create_list(name)
    DB[:lists].insert(:name => name)
  end
  
  def delete_list(id)
    DB[:todos].where(:list_id => id).delete
    DB[:lists].where(:id => id).delete
  end

  def find_list(id)
    all_lists.where(:lists__id => id).first
  end
  
  def all_lists
    DB[:lists].left_join(:todos, list_id: :id).
      select_all(:lists).
      select_append do
        [ count(todos__id).as(todos_count), 
          count(nullif(todos__is_completed, true)).as(todos_remaining_count) ]
      end.
      group(:lists__id).
      order(:lists__name)
  end
  
  def update_list_name(id, new_name)
    DB[:lists].where(:id => id).update(:name => new_name)
  end
  
  def create_new_todo(list_id, todo_name)
    DB[:todos].insert(:name => todo_name, :list_id => list_id)
  end
  
  def delete_todo_from_list(list_id, todo_id)
    DB[:todos].where(:list_id => list_id).delete
  end
  
  def update_todo_status(list_id, todo_id, new_status)
    DB[:todos].where(:id => todo_id, :list_id => list_id).update(:is_completed => new_status)
  end
  
  def mark_all_todos_as_completed(list_id)
    DB[:todos].where(:list_id => list_id).update(:is_completed => true)
  end
  
  def find_todos_for_list(list_id)
    DB[:todos].select do
      [ id, name, is_completed.as(:completed)]
    end.where(:list_id => list_id)
  end
end